     ...
     if(convergenceTest){
       if(tmpmod$fit$convergence!=0){
         ...
         conv_table <- data.frame(rbind(
           ...
           c(10,"degeneracy of the Nelder–Mead simplex (10)"),
         ...
         message_out <- conv_table%>%filter(code== tmpmod$fit_optim$convergence)%>%select(message)%>%as.character()
         tmpmod$convergence_test = c(nlminb = tmpmod$fit$message, optim = message_out)
         tmpmod$convergence_test_n = c(nlminb = tmpmod$fit$convergence, optim = tmpmod$fit_optim$convergence)
      }
     }
   }
  if(optimizer == "optim"){
